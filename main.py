...
def main(image_vectors, method, ethnicity_2, database, CAM):
    '''
    statistics derived from high quality samples
    '''
    print(method, ethnicity_2, database, CAM)

    ethnicity_1 = 'Caucasian'

    output_dir = "./results/{}/{}/{}".format(method, database, CAM)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    base_root_1 = r"./data/{}/{}/{}".format(method, database, CAM)
    data = np.load("{}/{}.npy".format(base_root_1, ethnicity_1))
   ...
   '''
    base_root_1 = r"./data/{}/{}/{}".format(method, database, CAM)
    data = np.load("{}/{}.npy".format(base_root_1, ethnicity_2))
    ...
    if 1:
        ...
        '''
        plot_qos_shift(torch.flatten(data_high_std), torch.flatten(data_low_std), method="{}/{}_{}_diff_mean_std".format(output_dir, ethnicity_1, ethnicity_2), \
    ...
    if 1:
        ...
        plot_2d_heatmap(get_heatmap(torch.unsqueeze(diff_avg.div(torch.max(diff_avg)), 0)), title="{}/{}_{}_diff_std_mean".format(output_dir, ethnicity_1, ethnicity_2))
        ...
        plot_2d_heatmap(hmp, title="{}/{}_std_sample".format(output_dir, ethnicity_1))
        hmp = 0.5 * get_heatmap(torch.unsqueeze(data_low_mean.div(torch.max(data_low_mean)), 0)) + 1.0 * img_org
        plot_2d_heatmap(hmp, title="{}/{}_std_sample".format(output_dir, ethnicity_2))

if __name__ == '__main__':

    image_vectors = {"RFW_Caucasian" : r"E:\RFW_aligned\test\data\Caucasian\m.0b775g9\m.0b775g9_0003.jpg",
                    "RFW_Asian" : r"E:\RFW_aligned\test\data\Asian\m.0bwgpk6\m.0bwgpk6_0004.jpg",
                    "RFW_African" : r"E:\RFW_aligned\test\data\African\m.0b6h44g\m.0b6h44g_0001.jpg",
                    "RFW_Indian" : r"E:\RFW_aligned\test\data\Indian\m.0b74ztv\m.0b74ztv_0002.jpg",
                    "BFW_Caucasian" : r"E:\BFW_aligned\white_males\n001063\0004_01.jpg",
                    "BFW_African" : r"E:\BFW_aligned\black_females\n000771\0230_01.jpg",
                    "BFW_Asian" : r"E:\BFW_aligned\asian_males\n004467\0677_01.jpg",
                    "BFW_Indian" : r"E:\BFW_aligned\indian_females\n003984\0186_01.jpg"}

    for ethnicity_2 in ['Asian', 'African', 'Indian']:
        for method in ["r50", "r100"]:
            for database in ["BFW", "RFW"]:
                for CAM in ["data_ScoreCAM"]:
                    main(image_vectors, method, ethnicity_2, database, CAM)
